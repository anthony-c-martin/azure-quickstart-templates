...
var keyVaultName_var = 'fw-quick-${uniqueString(subscription().id, resourceGroup().id)}'
...
resource keyVaultName_keyVaultCASecretName 'Microsoft.KeyVault/vaults/secrets@2019-09-01' = {
  parent: keyVaultName
...
resource WorkerVM_Bootstrap 'Microsoft.Compute/virtualMachines/extensions@2020-06-01' = {
  ...
  properties: {
    ...
    settings: {
      commandToExecute: 'echo ${reference('CreateAndDeployCertificates').outputs.rootca} > c:\\root.pem.base64 && powershell "Set-Content -Path c:\\root.pem -Value ([Text.Encoding]::UTF8.GetString([convert]::FromBase64String((Get-Content -Path c:\\root.pem.base64))))" && certutil -addstore root c:\\root.pem'
    }
  }
  dependsOn: [
    CreateAndDeployCertificates
...
resource DemoFirewallPolicy 'Microsoft.Network/firewallPolicies@2020-07-01' = {
  ...
  properties: {
    ...
    transportSecurity: {
      certificateAuthority: {
        name: keyVaultCASecretName
        keyVaultSecretId: '${reference(keyVaultName.id, '2019-09-01').vaultUri}secrets/${keyVaultCASecretName}'
      }
    }
    intrusionDetection: {
      mode: 'Alert'
  ...
  dependsOn: [
    keyVaultName_keyVaultCASecretName
  ]
}
