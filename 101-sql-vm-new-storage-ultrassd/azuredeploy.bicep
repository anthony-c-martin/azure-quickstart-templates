...
resource virtualMachineName_dataDisk_UltraSSD 'Microsoft.Compute/disks@2019-11-01' = [for i in range(0, 1): {
...
resource virtualMachineName_dataDisk 'Microsoft.Compute/disks@2019-11-01' = [for i in range(0, sqlDataDisksCount): {
...
resource virtualMachineName_resource 'Microsoft.Compute/virtualMachines@2019-12-01' = {
  ...
  properties: {
    ...
    storageProfile: {
      ...
      dataDisks: [for j in range(0, (sqlDataDisksCount + 1)): {
        lun: j
        createOption: 'Attach'
        caching: ((j >= sqlDataDisksCount) ? 'None' : dataDisks.caching)
        managedDisk: {
          id: ((j >= sqlDataDisksCount) ? resourceId('Microsoft.Compute/disks/', '${virtualMachineName}-dataDisk-UltraSSD-0') : resourceId('Microsoft.Compute/disks/', '${virtualMachineName}-dataDisk-${j}'))
        }
      }]
    }
    networkProfile: {
  ...
  dependsOn: [
    virtualMachineName_dataDisk_UltraSSD
    virtualMachineName_dataDisk
  ]
}
