...
param toolVmName string = 'SuricataIDS'
...
param artifactsLocation string = 'https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/cloudlens-suricata-ubuntu/'
...
var toolPublicIPAddressName_var = 'suricataPublicIP'
var toolStorageAccountName_var = 'tooldisk${uniqueString(resourceGroup().id)}'
var toolDnsLabelPrefix = 'suricata-${uniqueString(resourceGroup().id)}'
var toolNicName_var = 'SuricataVMNic'
...
resource tapVmName_resource 'Microsoft.Compute/virtualMachines@2017-03-30' = {
  ...
  properties: {
    ...
    diagnosticsProfile: {
      bootDiagnostics: {
        enabled: true
        storageUri: reference(tapStorageAccountName.id, '2018-02-01').primaryEndpoints.blob
      }
    }
  }
}
...
resource toolVmName_config 'Microsoft.Compute/virtualMachines/extensions@2017-03-30' = {
  ...
  properties: {
    ...
    protectedSettings: {
      fileUris: [
        uri(artifactsLocation, 'scripts/setup-suricata.sh${artifactsLocationSasToken}')
      ]
      commandToExecute: './setup-suricata.sh ${cloudlensProjectKey}'
    }
  }
}
...
resource tapVmName_config 'Microsoft.Compute/virtualMachines/extensions@2017-03-30' = {
  ...
  properties: {
    ...
    protectedSettings: {
      fileUris: [
        uri(artifactsLocation, 'scripts/setup-tap.sh${artifactsLocationSasToken}')
      ]
...
output Suricata_SSH_Command string = 'ssh ${adminUsername}@${reference(toolPublicIPAddressName_var).dnsSettings.fqdn}'
output Suricata_Host_Name string = reference(toolPublicIPAddressName_var).dnsSettings.fqdn
