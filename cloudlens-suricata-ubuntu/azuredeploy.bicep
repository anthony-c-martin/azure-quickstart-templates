...
param artifactsLocation string {
  ...
  default: 'https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/cloudlens-suricata-ubuntu/'
}
...
var toolPublicIPAddressName = 'suricataPublicIP'
...
resource tapVmName_resource 'Microsoft.Compute/virtualMachines@2017-03-30' = {
  ...
  properties: {
    ...
    diagnosticsProfile: {
      bootDiagnostics: {
        enabled: true
        storageUri: reference(tapStorageAccountName_resource.id, '2018-02-01').primaryEndpoints.blob
      }
    }
  }
  dependsOn: [
...
output Suricata_SSH_Command string = 'ssh ${adminUsername}@${reference(toolPublicIPAddressName).dnsSettings.fqdn}'
output Suricata_Host_Name string = reference(toolPublicIPAddressName).dnsSettings.fqdn
