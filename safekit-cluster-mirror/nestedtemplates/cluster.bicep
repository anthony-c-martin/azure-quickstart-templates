...
var lbPoolID = ((Loadbalancer == 'none') ? '' : resourceId('Microsoft.Network/loadBalancers/backendAddressPools', lbName, lbPoolName))
var lbaddrpool1 = [
  {
    id: lbPoolID
  }
...
module optionalLB './nested_optionalLB.bicep' = {
  name: 'optionalLB'
  params: {
    resourceId_Microsoft_Network_publicIPAddresses_concat_pip_parameters_VIPDnsLabel: resourceId('Microsoft.Network/publicIPAddresses', 'pip${VIPDnsLabel}')
    resourceId_Microsoft_Network_loadBalancers_frontendIPConfigurations_variables_lbName_variables_frontEndIPConfigName: resourceId('Microsoft.Network/loadBalancers/frontendIPConfigurations', lbName, frontEndIPConfigName)
    resourceId_Microsoft_Network_loadBalancers_backendAddressPools_variables_lbName_variables_lbPoolName: resourceId('Microsoft.Network/loadBalancers/backendAddressPools', lbName, lbPoolName)
    resourceId_Microsoft_Network_loadBalancers_probes_variables_lbName_variables_lbProbeName: resourceId('Microsoft.Network/loadBalancers/probes', lbName, lbProbeName)
    variables_lbName: lbName
    variables_frontEndIPConfigName: frontEndIPConfigName
    variables_lbPoolName: lbPoolName
    variables_lbProbeName: lbProbeName
    VIPDnsLabel: VIPDnsLabel
...
resource vms 'Microsoft.Compute/virtualMachines@2018-06-01' = [for (item, i) in vms_var: {
  ...
  properties: {
    ...
    diagnosticsProfile: {
      bootDiagnostics: {
        enabled: true
        storageUri: reference(resourceId(resourceGroup().name, 'Microsoft.Storage/storageAccounts/', storageAccountName_var)).primaryEndpoints.blob
      }
    }
  }
  dependsOn: [
...
module safekitClusterConfig '?' /*TODO: replace with correct path to [variables('skclustertemplate')]*/ = {
  ...
  params: {
    ...
    vmname: vms_var[0]
    vmList: vms_var
    fqdn: reference(resourceId(resourceGroup().name, 'Microsoft.Network/publicIPAddresses/', 'pip${vms_var[0]}')).dnsSettings.fqdn
    privateIps: ips
...
output fqdn string = reference(resourceId(resourceGroup().name, 'Microsoft.Network/publicIPAddresses/', 'pip${vms_var[0]}')).dnsSettings.fqdn
output Credentials_Url string = 'https://${reference(resourceId(resourceGroup().name, 'Microsoft.Network/publicIPAddresses/', 'pip${vms_var[0]}')).dnsSettings.fqdn}:9001/adduser.html'
output Credentials_Url_Login string = 'user: CA_admin, password: the value of AdminPassword'
output Console_Url string = 'https://${reference(resourceId(resourceGroup().name, 'Microsoft.Network/publicIPAddresses/', 'pip${vms_var[0]}')).dnsSettings.fqdn}:9453/deploy.html'
