...
param artifactsLocation string = 'https://raw.githubusercontent.com/Azure/azure-quickstart-templates/master/sccm-currentbranch/'
...
var siteRole = ((configuration == 'Standalone') ? createArray('PS') : createArray('CS', 'PS'))
...
resource prefix_vmInfo_vmInfo_vmRole_name_siteInfo_siteRole_vmInfo_name_WorkFlow 'Microsoft.Compute/virtualMachines/extensions@2019-12-01' = [for i in range(0, (length(vmInfo) + length(siteInfo))): {
  name: '${toLower(prefix)}${((i < length(vmInfo)) ? vmInfo[vmrole[i]].name : siteInfo[siteRole[(i - length(vmInfo))]].name)}/WorkFlow'
  location: location
  properties: {
    ...
    settings: {
      ...
      configurationFunction: '${((i < length(vmInfo)) ? vmrole[i] : siteRole[(i - length(vmInfo))])}Configuration.ps1\\Configuration'
      Properties: {
        ...
        ClientName: concat(prefix, vmInfo.Client.name)
  ...
  dependsOn: [
    resourceId('Microsoft.Compute/virtualMachines/', concat(toLower(prefix), ((i < length(vmInfo)) ? vmInfo[vmrole[i]].name : siteInfo[siteRole[(i - length(vmInfo))]].name)))
  ]
}]
...
resource prefix_vmInfo_vmInfo_vmRole_name_siteInfo_siteRole_vmInfo_name_ni 'Microsoft.Network/networkInterfaces@2020-05-01' = [for i in range(0, (length(vmInfo) + length(siteInfo))): {
  name: '${toLower(prefix)}${toLower(((i < length(vmInfo)) ? vmInfo[vmrole[i]].name : siteInfo[siteRole[(i - length(vmInfo))]].name))}-ni'
  location: location
  properties: {
    ipConfigurations: [
      {
        ...
        properties: {
          ...
          publicIPAddress: {
            id: resourceId('Microsoft.Network/publicIpAddresses', '${toLower(prefix)}${toLower(((i < length(vmInfo)) ? vmInfo[vmrole[i]].name : siteInfo[siteRole[(i - length(vmInfo))]].name))}-ip')
          }
        }
      }
    ]
  ...
  dependsOn: [
    virtualNetworkName
    resourceId('Microsoft.Network/publicIpAddresses/', '${toLower(prefix)}${toLower(((i < length(vmInfo)) ? vmInfo[vmrole[i]].name : siteInfo[siteRole[(i - length(vmInfo))]].name))}-ip')
    resourceId('Microsoft.Network/networkSecurityGroups/', '${toLower(toLower(prefix))}-nsg')
  ]
}]

resource prefix_vmInfo_vmInfo_vmRole_name_siteInfo_siteRole_vmInfo_name_ip 'Microsoft.Network/publicIpAddresses@2020-05-01' = [for i in range(0, (length(vmInfo) + length(siteInfo))): {
  name: '${toLower(prefix)}${toLower(((i < length(vmInfo)) ? vmInfo[vmrole[i]].name : siteInfo[siteRole[(i - length(vmInfo))]].name))}-ip'
  location: location
  properties: {
