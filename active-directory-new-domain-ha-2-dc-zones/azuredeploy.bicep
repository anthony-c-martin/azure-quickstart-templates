...
var adBDCConfigurationModulesURL = uri(artifactsLocation, 'DSC/ConfigureADBDC.ps1.zip')
...
resource nicName 'Microsoft.Network/networkInterfaces@2019-02-01' = [for i in range(0, 2): {
  name: nicName_var[i]
  location: location
  properties: {
    ipConfigurations: [
...
resource vmName 'Microsoft.Compute/virtualMachines@2019-03-01' = [for i in range(0, 2): {
  name: vmName_var[i]
  location: location
  zones: [
    (i + 1)
...
resource vmName_0_CreateAdForest 'Microsoft.Compute/virtualMachines/extensions@2019-03-01' = {
  ...
  properties: {
    ...
    settings: {
      configuration: {
        url: uri(artifactsLocation, 'DSC/CreateADPDC.ps1.zip')
...
resource vmName_1_PepareBDC 'Microsoft.Compute/virtualMachines/extensions@2019-03-01' = {
  ...
  properties: {
    ...
    settings: {
      configuration: {
        url: uri(artifactsLocation, 'DSC/PrepareADBDC.ps1.zip')
